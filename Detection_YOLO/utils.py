...
def parse_annotation(ann_dir):
    """ Returns 
    [{'filename': '2008_000200',
      'height': 3,
      'object': [{'name': 'text','xmax': 120,'xmin': 78,'ymax': 136,'ymin': 82},
         {'name': 'text', 'xmax': 74, 'xmin': 41, 'ymax': 190, 'ymin': 151}],
      'width': 8}]"""
    all_img = []
...
def aug_img(filename, wordBB, text):
    path = SYNTH_IMAGES + filename
    #print(path)
    img = cv2.imread(path)
    h, w, c = img.shape
    ...
    #TODO Check if there are any parallelograms in Synth
    for b_index in range(wordBB.shape[-1]): #Last dimension is for num of boxes
        obj = {}
        obj['name'] = text[b_index]
        
        #Translate Synth cordinates(4*x*y, all four points) to 
        #VOC style coordinates(2*x*y = left/top and bottom,right)
        x_min_index = 0
        y_min_index = 0
        x_max_index = 2
        y_max_index = 2        
...
def data_gen(im_names, wordBB, text, batch_size):
    """ all_img = """
    num_img = len(im_names)
    ...
    while True:
        ...
        currt_inst = 0
        x_batch = np.zeros((batch_size, NORM_W, NORM_H, 3)) #Input size
        y_batch = np.zeros((batch_size, GRID_W, GRID_H, BOX, 5+CLASS)) #OP tensor
        
        for index in shuffled_indices[l_bound:r_bound]:
            
            # augment input image and fix object's position and size
            img, all_obj = aug_img(im_names[index][0], wordBB[index], text[index])
            #for obj in all_obj:
            ...
            for obj in all_obj:    
                ...
                if grid_x < GRID_W and grid_y < GRID_H:
                    obj_indx = 0#LABELS.index(obj['name'])
